...
export const selectProductsCacheByKey = (key: string) => (state: RootState) =>
  state.products.cache[key]?.items ?? [];

export const selectProductsLastFetchedAtByKey =
  (key: string) => (state: RootState) =>
    state.products.cache[key]?.lastFetchedAt ?? null;

export const selectProductsStatusByKey = (key: string) => (state: RootState) =>
  state.products.statusByKey[key] ?? 'idle';
